...
const requestsStorageKey = 'sagi_requests';
...
const requesterOptions = ['Admin SAGI', 'Maria Gomez', 'Carlos Ramirez', 'Laura Torres', 'Andres Molina', 'Natalia Rios'];
...
const createEmptyRequestProduct = () => ({
  productCode: '',
  lot: '',
  quantity: '',
  observations: '',
...
function RequestField({ children, error, label, required }) {
  return (
    <label className="product-field request-form-field">
      <span>
...
function validateRequestForm(form, rows) {
  ...
  if (!form.justification.trim()) errors.justification = 'Describe el motivo de la solicitud.';
  if (rows.length === 0) errors.products = 'Debe existir al menos un producto en la solicitud.';

  rows.forEach((row, index) => {
    if (!row.productCode) rowErrors[index].productCode = 'Selecciona un producto.';
  ...
  return {
    ...
    rowErrors,
    hasErrors:
      Object.keys(errors).length > 0 || rowErrors.some((rowError) => Object.keys(rowError).length > 0),
  };
}
